...
import LauchList from "../../components/Launches/Launches";
...
export class SpaceLaunch extends React.Component {
  state = {
    filterData: [
      {
        type: "launch_year",
  ...
  componentDidMount() {
    this.props.fetchLaunchData(
      `?limit=${this.state.dataLimit}${this.state.queryString && "&" + this.state.queryString}`
    );
  }
...
export default connect(mapStateToProps, { fetchLaunchData })(SpaceLaunch);
